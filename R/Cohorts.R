...
createCohorts <- function(executionSettings = NULL, ...) {
  ...
  if (is.null(executionSettings$cohortDefinitionSet)) {
    executionSettings$cohortDefinitionSet <- CohortGenerator::createEmptyCohortDefinitionSet()
  }
  ...
  if (executionSettings$useBulkCohorts) {
    ...
    cohrtRef <- cohrtRef |>
      dplyr::bind_rows(rxNormRefs |>
        dplyr::select("cohortId", "cohortName") |>
        dplyr::mutate(atcFlag = 0, conceptId = .data$cohortId/1000, shortName = .data$cohortName) |>
        dplyr::rename(cohortDefinitionName = "cohortName", cohortDefinitionId = "cohortId"))
    ...
    cohrtRef <- cohrtRef |>
      dplyr::bind_rows(atcRefs |>
        dplyr::select("cohortId", "cohortName") |>
        dplyr::mutate(atcFlag = 1, conceptId = .data$cohortId/1000, shortName = .data$cohortName) |>
        dplyr::rename(cohortDefinitionName = "cohortName", cohortDefinitionId = "cohortId"))
  ...
  for (subsetDef in executionSettings$indicationCohortSubsetDefintions) {
    executionSettings$cohortDefinitionSet <- executionSettings$cohortDefinitionSet |>
      CohortGenerator::addCohortSubsetDefinition(subsetDef,
                                                 targetCohortIds = cohrtRef$cohortDefinitionId)
  }
  ...
  cohrtRef <- cohrtRef |>
    dplyr::bind_rows(executionSettings$cohortDefinitionSet |>
      ...
      dplyr::mutate(atcFlag = -1, conceptId = -1, shortName = .data$cohortName) |>
      dplyr::rename(cohortDefinitionName = "cohortName", cohortDefinitionId = "cohortId"))
