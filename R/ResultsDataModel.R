...
#' It is strongly advised that you have a backup of all data (either sqlite files, a backup database
#' (in the case you are using a postgres backend) or have kept the csv/zip files from your data
#' generation.
#'
#' @inheritParams
#' getDataMigrator
...
migrateDataModel <- function(connectionDetails, databaseSchema, tablePrefix = "") {
  ...
  updateVersionSql <- SqlRender::loadRenderTranslateSql("UpdateVersionNumber.sql",
    ...
    database_schema = databaseSchema, table_prefix = tablePrefix, dbms = connectionDetails$dbms)
...
getDataMigrator <- function(connectionDetails, databaseSchema, tablePrefix = "") {
  ResultModelManager::DataMigrationManager$new(connectionDetails = connectionDetails,
    ...
    tablePrefix = tablePrefix, migrationPath = "migrations", packageName = utils::packageName())
}
...
#' @param importFilpath       file path to export zipped results to before upload (optional - default
#'                            is temporary)
  ...
  if (connectionDetails$dbms == "postgresql") {
    ...
    for (databaseId in databaseIds) {
      DatabaseConnector::renderTranslateExecuteSql(connection,
        ...
        database_id = databaseId, table_prefix = tablePrefix)
    }
  }
  ResultModelManager::uploadResults(connectionDetails = connectionDetails, schema = databaseSchema,
    resultsFolder = importFilpath, tablePrefix = tablePrefix, specifications = getResultsDataModelSpec(),
    ...)
}
